...
class AttachmentPipeline(
    ...
    companion object { const val DEFAULT_MAX_BYTES = 50L * 1_024 * 1_024 }
}

private fun String.toSupportedKind(): AttachmentKind? = when (lowercase()) {
    "text/plain" -> AttachmentKind.TEXT
    "application/pdf" -> AttachmentKind.PDF
    "text/csv", "application/csv" -> AttachmentKind.CSV
    "application/json", "text/json" -> AttachmentKind.JSON
    "image/jpeg", "image/png", "image/webp" -> AttachmentKind.IMAGE
    else -> null
}
